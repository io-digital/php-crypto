<?php
/**
 * Crypto is a library for eleptic curve cryptography in PHP7
 * @package
 * @author      Werner Roets <werner@io.co.za>
 * @license     TBD
 * @version     0.1
 * @link        http://githubrepowillgohere
 */

 /**
  * Prime.php
  * As laid out in:
  * More Modular Exponential (MODP) Diffie-Hellman groups
  * for Internet Key Exchange (IKE)
  * http://tools.ietf.org/html/rfc3526
  * @author     Werner Roets <werner@io.co.za>
  * @copyright  2016
  * @license    TBD
  * @link       http://githubrepowillgohere
  */
class Prime {
    /*

     */
    const BIT_1563 = 0;
    const BIT_2048 = 1;
    const BIT_3072 = 2;

    const PRIME = [
        // 1563-bit
        '',
        // 2048-bit
        '',
        // 3072-bit
        '',
    ];

    public static function asGMP(int $prime_size) : GMP {
        return gmp_init(self::PRIME[$prime_size]);
    }

    public static function asIntString(int $prime) : string {
        return gmp_strval(gmp_init(self::PRIME[$prime_size]));
    }

    public static function asHexString(int $prime, bool $prefixed = true) : string {
        if($prefixed){
            return self::PRIME[$prime_size];
        } else {
            return substr(self::PRIME[$prime_size], 2);
        }
    }

}
